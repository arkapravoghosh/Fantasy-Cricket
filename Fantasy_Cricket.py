...
class Ui_MainWindow(object):
    ...
    def bat(self):
        try:
            ...
            msg.setText("Team not created. Please create a new team.")
            ...
            self.listWidget.addItem("")
            for i in l:
                if i[1] == 'BAT' and i[0] not in SelectedPlayers:
                    s = i[0]
                    CurrentPlayers.append(s)
                    ls = len(s)
                    t = str(i[2]).rjust(30 - ls)
    ...
    def bow(self):
        try:
            ...
            t = t[0]
            c.execute("DELETE FROM Teams WHERE player1 = ''")
            self.team_name.setText(_translate("MainWindow", "{}".format(t)))
            self.listWidget.clear()
            c.execute("SELECT * from PlayerDetails")
            l = c.fetchall()
            global CurrentPlayers
            ...
            self.listWidget.addItem("")
            for i in l:
                if i[1] == 'BWL' and i[0] not in SelectedPlayers:
                    s = i[0]
                    CurrentPlayers.append(s)
                    ls = len(s)
                    t = str(i[2]).rjust(30 - ls)
    ...
    def ar(self):
        ...
        self.listWidget.addItem("")
        for i in l:
            if i[1] == 'AR' and i[0] not in SelectedPlayers:
                s = i[0]
                CurrentPlayers.append(s)
                ls = len(s)
                t = str(i[2]).rjust(30 - ls)
    ...
    def wk(self):
        ...
        self.listWidget.addItem("")
        for i in l:
            if i[1] == 'WK' and i[0] not in SelectedPlayers:
                s = i[0]
                CurrentPlayers.append(s)
                ls = len(s)
                t = str(i[2]).rjust(30 - ls)
    ...
    def PlayerClicked(self, item):
        ...
        conn = sqlite3.connect('Matches.db')
        c = conn.cursor()
        c.execute("SELECT * from PlayerDetails WHERE name = ?", (t,))
        l = c.fetchone()
        category = l[1]
        value = l[2]
        #print(category)
        ...
        if no_of_bat + no_of_bwl + no_of_ar + no_of_wk == 11:
            msg = QtWidgets.QMessageBox()
            msg.setIcon(QtWidgets.QMessageBox.Warning)
...
"background-color: rgb(240, 240, 240);")
        ...
        ''''''
        self.listWidget.itemDoubleClicked.connect(self.PlayerClicked)
        ''''''
        ...
        ''''''
        self.bow_btn.toggled.connect(self.bow)
        ''''''
